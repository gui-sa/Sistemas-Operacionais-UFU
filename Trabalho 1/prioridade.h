...
void prioridadeNP(int N, processo *p){
    ...
    for (int i=0;i<N;i++){
        fprintf(arqNome,"Tempo entrada na fila do pronto para o processo [ID=%d] eh: %d ms\n",conta[i].ID,conta[i].t_enter);
        fprintf(arqNome,"Tempo de inicio de execucao para o processo [ID=%d] eh: %d ms\n",conta[i].ID,conta[i].t_init);
        fprintf(arqNome,"Tempo de retorno para o processo [ID=%d] eh: %d ms\n",conta[i].ID,conta[i].t_end);
    ...
    fprintf(arqNome,"\n\nTempo medio de inicio eh: %f ms\n",media_inicio);
    fprintf(arqNome,"Tempo medio de retorno eh: %f ms\n",media_retorno);
    fclose(arqNome);
    free(p);
    free(conta);
}
